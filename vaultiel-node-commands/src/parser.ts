import type { CLISubcommand, SubcommandOption } from "./types.js";
...
export function parseSubcommandArgs(
  ...
  // Build flag lookup
  const optionsByFlag = new Map<string, SubcommandOption>();
  for (const opt of subcmd.options) {
...
export function formatSubcommandHelp(subcmd: CLISubcommand): string {
  const lines: string[] = [];
  lines.push(`${subcmd.name} — ${subcmd.description}`);
  lines.push("");

  // Usage line
  const argParts = subcmd.args.map((a) =>
    a.required ? `<${a.name}>` : `[${a.name}]`,
  );
  const optParts = subcmd.options.map((o) => {
    if (o.type === "boolean") return `[${o.flag}]`;
...
export function formatSubcommandList(
  ...
  // Group by group name
  const groups = new Map<string, CLISubcommand[]>();
